...
//                            Toast.makeText(context, matchedRidesData.size() + ", "  + allMatchedRides.size(), Toast.LENGTH_LONG).show();
    ...
    */
    ...
    public static void find_a_ride(final OnSearchListener listener, final Context context, final String city_from, final String city_to, final String state_from,
                                   ...
                                   final String country_to, final long date_time, final long user_id){
        ...
        UserOperations.getInstance(context).signUp(UrlConstant.SOCIAL_SIGNUP, params, new OnLoadFinished() {
            ...
            public void onSuccess(JSONObject jsonObject) {
                try {
                    ...
                    Toast.makeText(context, e.getMessage() + "xcvxc", Toast.LENGTH_SHORT).show();
                }
            }
            ...
            public void onFail(String error) {
                ...
                if (i == 1) {
                    Toast.makeText(context, "GoogleLogout", Toast.LENGTH_LONG).show();
                    GooglePlusLogin googlePlusLogin = GooglePlusLogin.getInstance(null, context);
                    googlePlusLogin.Logout();
                    context.getSharedPreferences("pref", context.MODE_PRIVATE).edit().remove("network").commit();

                    System.exit(1);
                }
                Toast.makeText(context, "vvnb", Toast.LENGTH_SHORT).show();
            }
        });
    }
...
//Toast.makeText(context,""+all_messages.size(),Toast.LENGTH_LONG).show();
    ...
    public static void uploadImage(Context context, final long user_id, String image_str) {
        Map<String, String> params = new HashMap<>();
        params.put("user_id", String.valueOf(user_id));
        ...
        UserOperations.getInstance(context).uploadImage(params, new OnLoadFinished() {
            ...
            public void onSuccess(JSONObject jsonObject) throws JSONException {
                ...
                if (success) {
                    ...
                    UserDAO.updateUserPhoto(user_id, imageUrl);
                }
